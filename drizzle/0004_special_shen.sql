-- Migration: Add pitch_coaching, pre_call_intelligence, prospects, generated_emails tables
-- and update app_settings with local AI columns

-- Update app_settings: add local AI columns
ALTER TABLE `app_settings`
  ADD COLUMN IF NOT EXISTS `ollamaEndpoint` varchar(512) DEFAULT 'http://localhost:11434',
  ADD COLUMN IF NOT EXISTS `ollamaModel` varchar(128) DEFAULT 'llama3.1:8b',
  ADD COLUMN IF NOT EXISTS `whisperEndpoint` varchar(512) DEFAULT 'http://localhost:8001';

-- Update meetings: add transcriptText for direct paste input
ALTER TABLE `meetings`
  ADD COLUMN IF NOT EXISTS `transcriptText` mediumtext;

-- Create pitch_coaching table
...
-- Create pre_call_intelligence table
